...
locals {
  name = var.name == null ? "${var.names.product}-${var.names.environment}-${var.names.location}-vm" : var.name
...
resource "azurerm_public_ip" "pip" {
  count               = var.enable_public_ip_address == true ? var.instance_count : 0
  name                = "${local.name}${count.index}-ip"
  ...
  tags                = merge({ "ResourceName" = lower("pip-vm-${local.name}-${data.azurerm_resource_group.rg.location}-0${count.index + 1}") }, var.tags, )
...
resource "azurerm_network_security_group" "nsg" {
  count               = var.existing_network_security_group_id == null && var.enable_security_group == true ? 1 : 0
  name                = "${local.name}-nsg"
  resource_group_name = data.azurerm_resource_group.rg.name
  location            = data.azurerm_resource_group.rg.location
  tags                = merge({ "ResourceName" = lower("nsg_${local.name}_${data.azurerm_resource_group.rg.location}_in") }, var.tags, )
...
resource "azurerm_network_interface_security_group_association" "nsgassoc" {
  count                     = var.enable_security_group == false ? 0 : var.instance_count
...
resource "azurerm_network_interface" "nic" {
  ...
  name                          = "${local.name}${count.index}-nic"
  resource_group_name           = data.azurerm_resource_group.rg.name
  location                      = data.azurerm_resource_group.rg.location
  ...
  tags                          = merge({ "ResourceName" = var.instance_count == 1 ? lower("nic-${format("vm%s", lower(replace(local.name, "/[[:^alnum:]]/", "")))}") : lower("nic-${format("vm%s%s", lower(replace(local.name, "/[[:^alnum:]]/", "")), count.index + 1)}") }, var.tags, )

  ip_configuration {
    name                          = lower("ipconig-${format("vm%s%s", lower(replace(local.name, "/[[:^alnum:]]/", "")), count.index + 1)}")
    primary                       = true
...
resource "azurerm_proximity_placement_group" "appgrp" {
  ...
  name                = "${local.name}${count.index}-ppg"
  resource_group_name = data.azurerm_resource_group.rg.name
  location            = data.azurerm_resource_group.rg.location
  tags                = merge({ "ResourceName" = lower("proxigrp-${local.name}-${data.azurerm_resource_group.rg.location}") }, var.tags, )
...
resource "azurerm_availability_set" "aset" {
  ...
  managed                      = true
  tags                         = merge({ "ResourceName" = lower("avail-${local.name}-${data.azurerm_resource_group.rg.location}") }, var.tags, )
...
resource "azurerm_linux_virtual_machine" "linux_vm" {
  ...
  tags                            = merge({ "ResourceName" = var.instance_count == 1 ? local.name : format("%s%s", lower(replace(local.name, "/[[:^alnum:]]/", "")), count.index + 1) }, var.tags, )

  dynamic "admin_ssh_key" {
    for_each = var.disable_password_authentication ? [1] : []
    content {
      username   = var.admin_username
      public_key = var.ssh_key == null ? tls_private_key.rsa[0].public_key_openssh : var.ssh_key
    }
  }
...
resource "azurerm_windows_virtual_machine" "win_vm" {
  ...
  name                         = var.instance_count == 1 ? substr(local.name, 0, 15) : substr(format("%s%s", lower(replace(local.name, "/[[:^alnum:]]/", "")), count.index + 1), 0, 15)
  computer_name                = var.instance_count == 1 ? substr(local.name, 0, 15) : substr(format("%s%s", lower(replace(local.name, "/[[:^alnum:]]/", "")), count.index + 1), 0, 15)
  resource_group_name          = data.azurerm_resource_group.rg.name
  location                     = data.azurerm_resource_group.rg.location
  size                         = var.vm_size
  admin_username               = var.admin_username
...
resource "azurerm_managed_disk" "data_disk" {
  ...
  name                 = "${local.name}_DataDisk_${each.value.idx}"
  resource_group_name  = data.azurerm_resource_group.rg.name
  location             = data.azurerm_resource_group.rg.location
  storage_account_type = lookup(each.value.data_disk, "storage_account_type", "StandardSSD_LRS")
  create_option        = "Empty"
  disk_size_gb         = each.value.data_disk.disk_size_gb
  tags                 = merge({ "ResourceName" = "${local.name}_DataDisk_${each.value.idx}" }, var.tags, )
...
resource "azurerm_virtual_machine_extension" "omsagentwin" {
  count                      = var.deploy_log_analytics_agent && var.log_analytics_workspace_id != null && var.os_type == "windows" ? var.instance_count : 0
  name                       = var.instance_count == 1 ? "OmsAgentForWindows" : format("%s%s", "OmsAgentForWindows", count.index + 1)
...
resource "azurerm_virtual_machine_extension" "omsagentlinux" {
  count                      = var.deploy_log_analytics_agent && var.log_analytics_workspace_id != null && var.os_type == "linux" ? var.instance_count : 0
  name                       = var.instance_count == 1 ? "OmsAgentForLinux" : format("%s%s", "OmsAgentForLinux", count.index + 1)
...
resource "azurerm_monitor_diagnostic_setting" "nsg" {
  ...
  name                       = lower("nsg-${local.name}-diag")
